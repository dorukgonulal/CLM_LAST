...
openai.api_key = "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"
def tts(text):
    # Language in which you want to convert
    language = 'en'  # 'en' for English, 'tr' for Turkish, etc.
    # Creating an instance of gTTS
    speech = gTTS(text=text, lang=language, slow=False)
    # Saving the converted audio to a file
    speech.save("./server/static/assets/output.mp3")
    ...
    try:
        ...
        if(counter != 4):
            completion = openai.ChatCompletion.create(
                model="gpt-4-turbo",
                messages=[
                    {"role": "system", "content": f"Write a story about a young child named {user_input} who experiences a daily negative event in the park due to greenhouse gases. At the story's most critical moment, present three alternative choices. Progress the story based on these choices, pausing at critical moments to present three choices each time, repeating this process three times in total. Every time choices are presented, let us know what they are and wait for our response. If user write 4th prompt, finish the story logicly and meaningful."},
                    {"role": "user", "content":"You said that, " + history + ", "  + user_input}
                ]
            )
            # Yanıtı al
            ...
            completion = openai.ChatCompletion.create(
                model="gpt-4-turbo",
                messages=[
                    {"role": "system", "content": "Finish the story with happy end."},
